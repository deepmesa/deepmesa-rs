...
use crate::matrix::cmd::macros::simd_mul_assign;
...
impl<T> MulAssign<T> for ColMajorDataset<T>
where
    ...
    fn mul_assign(&mut self, val: T) {
        ...
        if self.is_transpose {
            iterate_col_major!(self, row, col, unsafe {
                cmd_mul_assign_t!(self, row, col, val);
            });
        } else {
            iterate_col_major!(self, row, col, unsafe {
                cmd_mul_assign!(self, row, col, val);
            });
        }
    }
}

impl<T> MulAssign<&ColMajorDataset<T>> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign,
{
    fn mul_assign(&mut self, rhs: &ColMajorDataset<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd_t, cmd_t, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign_t!(self, row, col, cmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd_t, cmd, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign_t!(self, row, col, cmd_get!(rhs, row, col));
                });
            }
        } else {
            if rhs.is_transpose {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd, cmd_t, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign!(self, row, col, cmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd, cmd, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign!(self, row, col, cmd_get!(rhs, row, col));
                });
            }
        }
    }
...
impl<T> MulAssign<&RowMajorDataset<T>> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign,
{
    fn mul_assign(&mut self, rhs: &RowMajorDataset<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd_t, rmd_t, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign_t!(self, row, col, rmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd_t, rmd, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign_t!(self, row, col, rmd_get!(rhs, row, col));
                });
            }
        } else {
            if rhs.is_transpose {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd, rmd_t, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign!(self, row, col, rmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::cmd::macros::simd_mul_assign!(cmd, rmd, self, rhs);
                iterate_col_major!(self, row, col, unsafe {
                    cmd_mul_assign!(self, row, col, rmd_get!(rhs, row, col));
                });
            }
        }
    }
...
impl<T> MulAssign<&DualIndexDataset<T>> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign,
{
...
//     macro_rules! lhs {
//         (cmd, $simd:ident, $t:ty) => {
//             col_major_dataset!([$t, 2, 3, $simd], 2,3,4;5,6,7)
//         };
...
//     fn_test_mul_assign_val!(test_mul_assign_cmd_val, cmd);
//     fn_test_mul_assign_val!(test_mul_assign_cmd_t_val, cmd_t);

//     fn_test_mul_assign!(test_mul_assign_cmd_rmd, cmd, rmd);
//     fn_test_mul_assign!(test_mul_assign_cmd_cmd, cmd, cmd);
//     fn_test_mul_assign!(test_mul_assign_cmd_did, cmd, did);
//     fn_test_mul_assign!(test_mul_assign_cmd_rmd_t, cmd, rmd_t);
//     fn_test_mul_assign!(test_mul_assign_cmd_cmd_t, cmd, cmd_t);
//     fn_test_mul_assign!(test_mul_assign_cmd_did_t, cmd, did_t);

//     fn_test_mul_assign!(test_mul_assign_cmd_t_rmd, cmd_t, rmd);
//     fn_test_mul_assign!(test_mul_assign_cmd_t_cmd, cmd_t, cmd);
//     fn_test_mul_assign!(test_mul_assign_cmd_t_did, cmd_t, did);
//     fn_test_mul_assign!(test_mul_assign_cmd_t_rmd_t, cmd_t, rmd_t);
//     fn_test_mul_assign!(test_mul_assign_cmd_t_cmd_t, cmd_t, cmd_t);
//     fn_test_mul_assign!(test_mul_assign_cmd_t_did_t, cmd_t, did_t);

//     fn_test_mul_assign_simd_val!(test_mul_assign_simd_cmd_val, cmd);
//     fn_test_mul_assign_simd_val!(test_mul_assign_simd_cmd_t_val, cmd_t);

//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_rmd, cmd, rmd);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_cmd, cmd, cmd);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_did, cmd, did);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_rmd_t, cmd, rmd_t);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_cmd_t, cmd, cmd_t);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_did_t, cmd, did_t);

//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_t_rmd, cmd_t, rmd);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_t_cmd, cmd_t, cmd);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_t_did, cmd_t, did);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_t_rmd_t, cmd_t, rmd_t);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_t_cmd_t, cmd_t, cmd_t);
//     fn_test_mul_assign_simd!(test_mul_assign_simd_cmd_t_did_t, cmd_t, did_t);
// }
