    ...
    T: MatrixElement + std::ops::AddAssign + std::ops::Add<Output = T>,
{
    fn add_into(&self, rhs: T, result: &mut MatrixColMajor<T>) {
        ...
        if self.is_transpose {
            ...
            if result.is_transpose {
                crate::matrix::cm::macros::simd_add_into!(cm, val, cm_t, self, rhs, result);
                iterate_row_major!(self, row, col, unsafe {
                    cm_assign_t!(result, row, col, cm_get!(self, row, col) + rhs);
                });
            } else {
                crate::matrix::cm::macros::simd_add_into!(cm, val, cm, self, rhs, result);
                iterate_row_major!(self, row, col, unsafe {
...
impl<T> AddInto<&MatrixColMajor<T>, MatrixColMajor<T>> for MatrixColMajor<T>
where
    T: MatrixElement + std::ops::AddAssign + std::ops::Add<Output = T>,
{
    fn add_into(&self, rhs: &MatrixColMajor<T>, result: &mut MatrixColMajor<T>) {
        debug_assert!(self.rows == result.rows);
        debug_assert!(self.cols == result.cols);
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_add_into!(cm_t, cm_t, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_add_into!(cm_t, cm_t, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
                ...
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_add_into!(cm_t, cm, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_add_into!(cm_t, cm, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
            ...
            if rhs.is_transpose {
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_add_into!(cm, cm_t, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_add_into!(cm, cm_t, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
                ...
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_add_into!(cm, cm, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_add_into!(cm, cm, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
...
mod tests {
    ...
    macro_rules! rhs {
        (cm, $simd:ident, $t:ty) => {
            {
                let mut cm = matrix_cm!([$t,3,2, $simd], 12,15;13,16;14,17);
                cm.transpose();
                cm
            }
        };
        (cm_t, $simd:ident, $t:ty) => {
            matrix_cm!([$t,2,3, false], 12,13,14;15,16,17)
        };
    }
    ...
    macro_rules! result {
        (cm, $simd:ident, $t:ty) => {
            matrix_cm!([$t, 2, 3, $simd], 17,19,21;23,25,27)
        };
        (cm_t, $simd:ident, $t:ty) => {
            {
                let mut cm = matrix_cm!([$t, 3, 2, $simd], 17,23;19,25;21,27);
                cm.transpose();
                cm
            }
        };
        (val, cm, $simd:ident, $t:ty) => {
            matrix_cm!([$t, 2, 3, $simd], 12,13,14;15,16,17)
        };
    ...
    fn_test_add_into_val_simd!(test_add_into_val_simd_cm_cm, cm, cm);
    fn_test_add_into_val_simd!(test_add_into_val_simd_cm_t_cm, cm_t, cm);
    fn_test_add_into_val_simd!(test_add_into_val_simd_cm_cm_t, cm, cm_t);
    fn_test_add_into_val_simd!(test_add_into_val_simd_cm_t_cm_t, cm_t, cm_t);
    ...
    fn_test_add_into_simd!(test_add_into_simd_cm_cm_cm, cm, cm, cm);
    fn_test_add_into_simd!(test_add_into_simd_cm_cm_cm_t, cm, cm, cm_t);

    fn_test_add_into_simd!(test_add_into_simd_cm_cm_t_cm, cm, cm_t, cm);
    fn_test_add_into_simd!(test_add_into_simd_cm_cm_t_cm_t, cm, cm_t, cm_t);

    fn_test_add_into_simd!(test_add_into_simd_cm_t_cm_cm, cm_t, cm, cm);
    fn_test_add_into_simd!(test_add_into_simd_cm_t_cm_cm_t, cm_t, cm, cm_t);

    fn_test_add_into_simd!(test_add_into_simd_cm_t_cm_t_cm, cm_t, cm_t, cm);
    fn_test_add_into_simd!(test_add_into_simd_cm_t_cm_t_cm_t, cm_t, cm_t, cm_t);
}
