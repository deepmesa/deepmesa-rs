...
impl<T> RedBlackTree<T> {
    ...
    pub fn clear(&mut self) {
        ...
        // Calculate queue capacity based on tree structure
        // For a binary tree with n nodes, the maximum width occurs at the bottom level
        // For a complete binary tree: max width = ceil(n/2)
        // For a red-black tree (height ≤ 2*log₂(n+1)), worst case width = 2^⌊2*log₂(n+1)⌋
        // However, in practice, we can use a more conservative estimate
        let queue_capacity = if self.len <= 4 {
            self.len
        } else {
            // Calculate based on tree height bounds for red-black trees
            let height_bound = (2.0 * ((self.len + 1) as f64).log2()).ceil() as usize;
            let max_level_width = 2_usize.pow((height_bound - 1).min(20) as u32); // Cap at 2^20 for safety

            // Use the minimum of theoretical max width and practical upper bound
            max_level_width.min(self.len / 2 + 1).max(8)
        };

        // Use iterative approach with queue to avoid stack overflow
        let mut queue = crate::CircularDeque::with_capacity(queue_capacity);
        queue.push_back(self.root);

        while let Some(node) = queue.pop_front() {
            unsafe {
                // Add children to queue before processing current node
...
mod tests {
    ...
    #[test]
    fn test_clear_queue_capacity_efficiency() {
        // Test that our queue capacity calculation is reasonable for different tree sizes
        let test_cases = vec![
            (1, "single node"),
            (2, "two nodes"),
            (4, "four nodes"),
        ...
        for (size, description) in test_cases {
            ...
            // Clear should complete efficiently without reallocating queue
            tree.clear();
    ...
    fn test_clear_capacity_calculation_bounds() {
        ...
        tree.insert(1);
        tree.insert(2);
        tree.clear(); // Should use len for capacity (2)
