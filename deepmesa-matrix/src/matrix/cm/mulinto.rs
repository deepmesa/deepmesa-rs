    ...
    fn mul_into(&self, rhs: T, result: &mut MatrixColMajor<T>) {
        ...
        if self.is_transpose {
            if result.is_transpose {
                ...
                crate::matrix::cm::macros::simd_mul_into!(cm_t, val, cm, self, rhs, result);
                iterate_row_major!(self, row, col, unsafe {
            ...
            if result.is_transpose {
                crate::matrix::cm::macros::simd_mul_into!(cm, val, cm_t, self, rhs, result);
                iterate_row_major!(self, row, col, unsafe {
                    cm_assign_t!(result, row, col, cm_get!(self, row, col) * rhs);
                });
            } else {
                crate::matrix::cm::macros::simd_mul_into!(cm, val, cm, self, rhs, result);
                iterate_row_major!(self, row, col, unsafe {
...
impl<T> MulInto<&MatrixColMajor<T>, MatrixColMajor<T>> for MatrixColMajor<T>
where
    T: MatrixElement + std::ops::MulAssign + std::ops::Mul<Output = T>,
{
    fn mul_into(&self, rhs: &MatrixColMajor<T>, result: &mut MatrixColMajor<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_mul_into!(cm_t, cm_t, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_mul_into!(cm_t, cm_t, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
                ...
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_mul_into!(cm_t, cm, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_mul_into!(cm_t, cm, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
            ...
            if rhs.is_transpose {
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_mul_into!(cm, cm_t, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_mul_into!(cm, cm_t, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
                ...
                if result.is_transpose {
                    crate::matrix::cm::macros::simd_mul_into!(cm, cm, cm_t, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign_t!(
                            result,
                            row,
                    ...
                    crate::matrix::cm::macros::simd_mul_into!(cm, cm, cm, self, rhs, result);
                    iterate_row_major!(self, row, col, unsafe {
                        cm_assign!(
                            result,
                            row,
...
mod tests {
    ...
    fn_test_mul_into_val_simd!(test_mul_into_val_simd_cm_cm, cm, cm);
    fn_test_mul_into_val_simd!(test_mul_into_val_simd_cm_t_cm, cm_t, cm);
    fn_test_mul_into_val_simd!(test_mul_into_val_simd_cm_cm_t, cm, cm_t);
    fn_test_mul_into_val_simd!(test_mul_into_val_simd_cm_t_cm_t, cm_t, cm_t);
    ...
    fn_test_mul_into_simd!(test_mul_into_simd_cm_cm_cm, cm, cm, cm);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_cm_cm_t, cm, cm, cm_t);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_cm_t_cm, cm, cm_t, cm);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_cm_t_cm_t, cm, cm_t, cm_t);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_t_cm_cm, cm_t, cm, cm);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_t_cm_cm_t, cm_t, cm, cm_t);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_t_cm_t_cm, cm_t, cm_t, cm);
    fn_test_mul_into_simd!(test_mul_into_simd_cm_t_cm_t_cm_t, cm_t, cm_t, cm_t);
}
