    ...
    fn add_assign(&mut self, rhs: &MatrixRowMajor<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                ...
                crate::matrix::rm::macros::simd_add_assign!(rm_t, rm, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
            ...
            if rhs.is_transpose {
                crate::matrix::rm::macros::simd_add_assign!(rm, rm_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rm_add_assign!(self, row, col, rm_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rm::macros::simd_add_assign!(rm, rm, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
...
mod tests {
    ...
    fn_test_add_assign_simd_val!(test_add_assign_simd_rm_val, rm);
    fn_test_add_assign_simd_val!(test_add_assign_simd_rm_t_val, rm_t);

    fn_test_add_assign_simd!(test_add_assign_simd_rm_rm, rm, rm);
    fn_test_add_assign_simd!(test_add_assign_simd_rm_rm_t, rm, rm_t);

    fn_test_add_assign_simd!(test_add_assign_simd_rm_t_rm, rm_t, rm);
    fn_test_add_assign_simd!(test_add_assign_simd_rm_t_rm_t, rm_t, rm_t);
}
