...
use crate::matrix::rmd::macros::simd_sub_assign;
...
impl<T> SubAssign<T> for RowMajorDataset<T>
where
    ...
    fn sub_assign(&mut self, val: T) {
        ...
        if self.is_transpose {
            iterate_row_major!(self, row, col, unsafe {
                rmd_sub_assign_t!(self, row, col, val);
            });
        } else {
            iterate_row_major!(self, row, col, unsafe {
                rmd_sub_assign!(self, row, col, val);
            });
        }
    }
}

impl<T> SubAssign<&RowMajorDataset<T>> for RowMajorDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
    fn sub_assign(&mut self, rhs: &RowMajorDataset<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd_t, rmd_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign_t!(self, row, col, rmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd_t, rmd, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign_t!(self, row, col, rmd_get!(rhs, row, col));
                });
            }
        } else {
            if rhs.is_transpose {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd, rmd_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign!(self, row, col, rmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd, rmd, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign!(self, row, col, rmd_get!(rhs, row, col));
                });
            }
        }
    }
...
impl<T> SubAssign<&ColMajorDataset<T>> for RowMajorDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
    fn sub_assign(&mut self, rhs: &ColMajorDataset<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd_t, cmd_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign_t!(self, row, col, cmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd_t, cmd, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign_t!(self, row, col, cmd_get!(rhs, row, col));
                });
            }
        } else {
            if rhs.is_transpose {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd, cmd_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign!(self, row, col, cmd_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rmd::macros::simd_sub_assign!(rmd, cmd, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rmd_sub_assign!(self, row, col, cmd_get!(rhs, row, col));
                });
            }
        }
    }
...
impl<T> SubAssign<&DualIndexDataset<T>> for RowMajorDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
...
mod tests {
    ...
    fn_test_sub_assign_val!(test_sub_assign_rmd_val, rmd);
    fn_test_sub_assign_val!(test_sub_assign_rmd_t_val, rmd_t);

    fn_test_sub_assign!(test_sub_assign_rmd_rmd, rmd, rmd);
    fn_test_sub_assign!(test_sub_assign_rmd_cmd, rmd, cmd);
    fn_test_sub_assign!(test_sub_assign_rmd_did, rmd, did);
    fn_test_sub_assign!(test_sub_assign_rmd_rmd_t, rmd, rmd_t);
    fn_test_sub_assign!(test_sub_assign_rmd_cmd_t, rmd, cmd_t);
    fn_test_sub_assign!(test_sub_assign_rmd_did_t, rmd, did_t);

    fn_test_sub_assign!(test_sub_assign_rmd_t_rmd, rmd_t, rmd);
    fn_test_sub_assign!(test_sub_assign_rmd_t_cmd, rmd_t, cmd);
    fn_test_sub_assign!(test_sub_assign_rmd_t_did, rmd_t, did);
    fn_test_sub_assign!(test_sub_assign_rmd_t_rmd_t, rmd_t, rmd_t);
    fn_test_sub_assign!(test_sub_assign_rmd_t_cmd_t, rmd_t, cmd_t);
    fn_test_sub_assign!(test_sub_assign_rmd_t_did_t, rmd_t, did_t);

    fn_test_sub_assign_simd_val!(test_sub_assign_simd_rmd_val, rmd);
    fn_test_sub_assign_simd_val!(test_sub_assign_simd_rmd_t_val, rmd_t);

    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_rmd, rmd, rmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_cmd, rmd, cmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_did, rmd, did);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_rmd_t, rmd, rmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_cmd_t, rmd, cmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_did_t, rmd, did_t);

    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_t_rmd, rmd_t, rmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_t_cmd, rmd_t, cmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_t_did, rmd_t, did);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_t_rmd_t, rmd_t, rmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_t_cmd_t, rmd_t, cmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_rmd_t_did_t, rmd_t, did_t);
}
