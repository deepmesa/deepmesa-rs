    ...
    T: MatrixElement + std::ops::MulAssign,
{
    fn mul_assign(&mut self, rhs: &MatrixRowMajor<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                crate::matrix::rm::macros::simd_mul_assign!(rm_t, rm_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rm_mul_assign_t!(self, row, col, rm_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rm::macros::simd_mul_assign!(rm_t, rm, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
            ...
            if rhs.is_transpose {
                crate::matrix::rm::macros::simd_mul_assign!(rm, rm_t, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
                    rm_mul_assign!(self, row, col, rm_get_t!(rhs, row, col));
                });
            } else {
                crate::matrix::rm::macros::simd_mul_assign!(rm, rm, self, rhs);
                iterate_row_major!(self, row, col, unsafe {
...
mod tests {
    ...
    macro_rules! result {
        (rm, $simd:ident, $t:ty) => {
            matrix_rm!([$t, 2, 3, $simd], 6,14,24;36,50,66)
        };
        (val, $simd:ident, $t:ty) => {
            matrix_rm!([$t, 2, 3, $simd], 3,6,9;12,15,18)
        };
    }
    ...
    fn_test_mul_assign_simd_val!(test_mul_assign_simd_rm_val, rm);
    fn_test_mul_assign_simd_val!(test_mul_assign_simd_rm_t_val, rm_t);

    fn_test_mul_assign_simd!(test_mul_assign_simd_rm_rm, rm, rm);
    fn_test_mul_assign_simd!(test_mul_assign_simd_rm_rm_t, rm, rm_t);

    fn_test_mul_assign_simd!(test_mul_assign_simd_rm_t_rm, rm_t, rm);
    fn_test_mul_assign_simd!(test_mul_assign_simd_rm_t_rm_t, rm_t, rm_t);
}
