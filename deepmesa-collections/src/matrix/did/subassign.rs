...
use crate::matrix::did::macros::simd_sub_assign;
...
impl<T> SubAssign<T> for DualIndexDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
    fn sub_assign(&mut self, val: T) {
        if self.use_simd() {
            ...
            let len = self.len();
            unsafe {
                SimdKernel::simd_sub_assign(ptr, val, len);
            }
        ...
        if self.is_transpose {
            iterate_row_major!(self, row, col, unsafe {
                rmd_sub_assign_t!(self.rmd, row, col, val);
                cmd_sub_assign_t!(self.cmd, row, col, val);
            });
        } else {
            iterate_row_major!(self, row, col, unsafe {
                rmd_sub_assign!(self.rmd, row, col, val);
                cmd_sub_assign!(self.cmd, row, col, val);
            });
        }
    }
}

impl<T> SubAssign<&RowMajorDataset<T>> for DualIndexDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
    fn sub_assign(&mut self, rhs: &RowMajorDataset<T>) {
        ...
        if self.is_transpose {
            if rhs.is_transpose {
                crate::matrix::rmd::macros::simd_sub_assign!(
                    rmd_t,
                    rmd_t,
                    self.rmd,
                    rhs,
                ...
                crate::matrix::cmd::macros::simd_sub_assign!(
                    cmd_t,
                    rmd,
                    self.cmd,
                    rhs,
...
impl<T> SubAssign<&ColMajorDataset<T>> for DualIndexDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
...
impl<T> SubAssign<&DualIndexDataset<T>> for DualIndexDataset<T>
where
    T: MatrixElement + std::ops::SubAssign,
{
...
mod tests {
    ...
    fn_test_sub_assign_val!(test_sub_assign_did_val, did);
    fn_test_sub_assign_val!(test_sub_assign_did_t_val, did_t);

    fn_test_sub_assign!(test_sub_assign_did_rmd, did, rmd);
    fn_test_sub_assign!(test_sub_assign_did_cmd, did, cmd);
    fn_test_sub_assign!(test_sub_assign_did_did, did, did);
    fn_test_sub_assign!(test_sub_assign_did_rmd_t, did, rmd_t);
    fn_test_sub_assign!(test_sub_assign_did_cmd_t, did, cmd_t);
    fn_test_sub_assign!(test_sub_assign_did_did_t, did, did_t);

    fn_test_sub_assign!(test_sub_assign_did_t_rmd, did_t, rmd);
    fn_test_sub_assign!(test_sub_assign_did_t_cmd, did_t, cmd);
    fn_test_sub_assign!(test_sub_assign_did_t_did, did_t, did);
    fn_test_sub_assign!(test_sub_assign_did_t_rmd_t, did_t, rmd_t);
    fn_test_sub_assign!(test_sub_assign_did_t_cmd_t, did_t, cmd_t);
    fn_test_sub_assign!(test_sub_assign_did_t_did_t, did_t, did_t);

    fn_test_sub_assign_simd_val!(test_sub_assign_simd_did_val, did);
    fn_test_sub_assign_simd_val!(test_sub_assign_simd_did_t_val, did_t);

    fn_test_sub_assign_simd!(test_sub_assign_simd_did_rmd, did, rmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_cmd, did, cmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_did, did, did);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_rmd_t, did, rmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_cmd_t, did, cmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_did_t, did, did_t);

    fn_test_sub_assign_simd!(test_sub_assign_simd_did_t_rmd, did_t, rmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_t_cmd, did_t, cmd);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_t_did, did_t, did);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_t_rmd_t, did_t, rmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_t_cmd_t, did_t, cmd_t);
    fn_test_sub_assign_simd!(test_sub_assign_simd_did_t_did_t, did_t, did_t);
}
