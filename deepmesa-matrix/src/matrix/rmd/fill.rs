...
impl<T> FillRow<&Vec<T>> for RowMajorDataset<T>
where
    T: MatrixElement<Output = T>,
{
...
impl<T> FillRow<(&RowMajorDataset<T>, MatrixDimension)> for RowMajorDataset<T>
where
    T: MatrixElement<Output = T>,
{
...
impl<T> FillRow<(&ColMajorDataset<T>, MatrixDimension)> for RowMajorDataset<T>
where
    T: MatrixElement<Output = T>,
{
    ...
    fn fill_row_rmd_row(&mut self, row: usize, rhs: &RowMajorDataset<T>, rhs_row: usize) {
        if self.is_transpose {
            if rhs.is_transpose {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign_t!(self, row, col, rmd_get_t!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            } else {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign_t!(self, row, col, rmd_get!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            }
        } else {
            if rhs.is_transpose {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign!(self, row, col, rmd_get_t!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            } else {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign!(self, row, col, rmd_get!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            }
        }
    ...
    fn fill_row_cmd_row(&mut self, row: usize, rhs: &ColMajorDataset<T>, rhs_row: usize) {
        if self.is_transpose {
            if rhs.is_transpose {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign_t!(self, row, col, cmd_get_t!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            } else {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign_t!(self, row, col, cmd_get!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            }
        } else {
            if rhs.is_transpose {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign!(self, row, col, cmd_get_t!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            } else {
                let mut rhs_col = 0;
                iterate_cols!(self, col, unsafe {
                    rmd_assign!(self, row, col, cmd_get!(rhs, rhs_row, rhs_col));
                    rhs_col += 1;
                });
            }
        }
