...
impl<T> Mul<T> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign + std::ops::Mul<Output = T>,
{
    type Output = ColMajorDataset<T>;
    fn mul(mut self, rhs: T) -> ColMajorDataset<T> {
        self.mul_assign(rhs);
        return self;
    }
}

impl<T> Mul<&RowMajorDataset<T>> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign + std::ops::Mul<Output = T>,
{
    type Output = ColMajorDataset<T>;
    fn mul(mut self, rhs: &RowMajorDataset<T>) -> ColMajorDataset<T> {
        self.mul_assign(rhs);
        return self;
    }
}

impl<T> Mul<&ColMajorDataset<T>> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign + std::ops::Mul<Output = T>,
{
    type Output = ColMajorDataset<T>;
    fn mul(mut self, rhs: &ColMajorDataset<T>) -> ColMajorDataset<T> {
        self.mul_assign(rhs);
        return self;
    }
}

impl<T> Mul<&DualIndexDataset<T>> for ColMajorDataset<T>
where
    T: MatrixElement + std::ops::MulAssign + std::ops::Mul<Output = T>,
{
    type Output = ColMajorDataset<T>;
    fn mul(mut self, rhs: &DualIndexDataset<T>) -> ColMajorDataset<T> {
        self.mul_assign(rhs);
        return self;
    }
}
...
{
    type Output = ColMajorDataset<T>;
    fn mul(self, rhs: T) -> ColMajorDataset<T> {
        let mut result = ColMajorDataset::from(&self);
...
{
    type Output = ColMajorDataset<T>;
    fn mul(self, rhs: &RowMajorDataset<T>) -> ColMajorDataset<T> {
        let mut result = ColMajorDataset::from(&self);
        self.mul_into(rhs, &mut result);
        return result;
    }
...
{
    type Output = ColMajorDataset<T>;
    fn mul(self, rhs: &ColMajorDataset<T>) -> ColMajorDataset<T> {
        let mut result = ColMajorDataset::from(&self);
        self.mul_into(rhs, &mut result);
        return result;
    }
...
{
    type Output = ColMajorDataset<T>;
    fn mul(self, rhs: &DualIndexDataset<T>) -> ColMajorDataset<T> {
        let mut result = ColMajorDataset::from(&self);
        self.mul_into(rhs, &mut result);
        return result;
    }
...
//     fn_test_mul_val!(test_mul_val_rmd, cmd);
//     fn_test_mul_val!(test_mul_val_rmd_t, cmd_t);

//     fn_test_mul_val_simd!(test_mul_simd_val_rmd, cmd);
//     fn_test_mul_val_simd!(test_mul_simd_val_rmd_t, cmd_t);

//     fn_test_mul!(test_mul_cmd_rmd, cmd, rmd);
//     fn_test_mul!(test_mul_cmd_cmd, cmd, cmd);
//     fn_test_mul!(test_mul_cmd_did, cmd, did);

//     fn_test_mul!(test_mul_cmd_rmd_t, cmd, rmd_t);
//     fn_test_mul!(test_mul_cmd_cmd_t, cmd, cmd_t);
//     fn_test_mul!(test_mul_cmd_did_t, cmd, did_t);

//     fn_test_mul!(test_mul_cmd_t_rmd, cmd_t, rmd);
//     fn_test_mul!(test_mul_cmd_t_cmd, cmd_t, cmd);
//     fn_test_mul!(test_mul_cmd_t_did, cmd_t, did);

//     fn_test_mul!(test_mul_cmd_t_rmd_t, cmd_t, rmd_t);
//     fn_test_mul!(test_mul_cmd_t_cmd_t, cmd_t, cmd_t);
//     fn_test_mul!(test_mul_cmd_t_did_t, cmd_t, did_t);

//     fn_test_mul_simd!(test_mul_simd_cmd_rmd, cmd, rmd);
//     fn_test_mul_simd!(test_mul_simd_cmd_cmd, cmd, cmd);
//     fn_test_mul_simd!(test_mul_simd_cmd_did, cmd, did);

//     fn_test_mul_simd!(test_mul_simd_cmd_rmd_t, cmd, rmd_t);
//     fn_test_mul_simd!(test_mul_simd_cmd_cmd_t, cmd, cmd_t);
//     fn_test_mul_simd!(test_mul_simd_cmd_did_t, cmd, did_t);

//     fn_test_mul_simd!(test_mul_simd_cmd_t_rmd, cmd_t, rmd);
//     fn_test_mul_simd!(test_mul_simd_cmd_t_cmd, cmd_t, cmd);
//     fn_test_mul_simd!(test_mul_simd_cmd_t_did, cmd_t, did);

//     fn_test_mul_simd!(test_mul_simd_cmd_t_rmd_t, cmd_t, rmd_t);
//     fn_test_mul_simd!(test_mul_simd_cmd_t_cmd_t, cmd_t, cmd_t);
//     fn_test_mul_simd!(test_mul_simd_cmd_t_did_t, cmd_t, did_t);
// }
